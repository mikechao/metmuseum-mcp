...
export class OpenMetExplorerAppResource {
  ...
  public async getHtml(): Promise<string> {
    ...
    const appWithDepsCode = await readFile(appWithDepsPath, 'utf-8');
    const appWithDepsBase64 = Buffer.from(appWithDepsCode, 'utf-8').toString('base64');
    this.htmlCache = buildOpenMetExplorerHtml(appWithDepsBase64);

    return this.htmlCache;
  }
}

function buildOpenMetExplorerHtml(appWithDepsBase64: string): string {
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
  ...
  <body>
    ...
    <script type="module">
      const appBundleUrl = 'data:text/javascript;base64,${appWithDepsBase64}';
      const {
        App,
        applyDocumentTheme,
        applyHostFonts,
        applyHostStyleVariables,
      } = await import(appBundleUrl);
      ...
      async function runSearch() {
        ...
        try {
          ...
          resultsTitleEl.textContent = parsed.total > 0
            ? 'Results (' + parsed.total + ' total, showing first 24)'
            : 'Results';
      ...
      function renderDetails() {
        ...
        const tagText = Array.isArray(objectData.tags)
          ? objectData.tags.map(tag => tag?.term).filter(Boolean).join(', ')
          : '';
        ...
        for (const line of lines) {
          ...
          switch (rawKey) {
            ...
            case 'Primary Image URL':
              ...
              break;
            case 'Tags':
              parsed.tags = value.split(',').map(term => ({ term: term.trim() })).filter(tag => tag.term);
              break;
            default:
              break;
          }
